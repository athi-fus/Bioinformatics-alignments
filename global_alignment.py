...
#print('Length of s2: {}'.format(len(s2)))
...
for i in range(1, len(s1)+1):
    arr[0][i]= int(arr[0][i-1]) + int(s(" ", s2[i-1])) #A

for j in range(1, len(s2)+1):
    arr[j][0]= int(arr[j-1][0]) + int(s(s1[j-1], " ")) #B
...
for i in range(1, len(s1)+1):
    for j in range(1, len(s2)+1):
        ...
        arr[i][j] = max(int(int(arr[i-1][j-1]) + int(s(s1[i-1], s2[j-1]))), int(int(arr[i-1][j]) + int(s(s1[i-1], " "))), int(int(arr[i][j-1]) + int(s(" ",s2[j-1]))))
'''
for i in range(len(s1)+1):
    for j in range(len(s1)+1):
        print(arr[i][j], end ="  ")
